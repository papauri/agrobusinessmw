...
<script src="assets/js/directory-navigation.js?v=20260814-2255"></script>
<script src="assets/js/app.js" defer></script>
<script src="assets/js/directory-home-hook.js?v=20260814-2255"></script>
...
<script>
    window.addEventListener('load', function () {
        if ('performance' in window && performance.getEntriesByType) {
            setTimeout(() => { const nav=performance.getEntriesByType('navigation')[0]; if(nav&&nav.loadEventEnd>0) console.log('Page load time:',Math.round(nav.loadEventEnd)+'ms'); },0);
        }
        if ('serviceWorker' in navigator) {
            const agroWorkerUrl = new URL('sw.js', window.location.href).href;
            navigator.serviceWorker.getRegistrations().then(regs => regs.forEach(reg => { if (reg.scriptURL === agroWorkerUrl) reg.unregister(); })).catch(() => {});
        }
    });
</script>
