...
(function () {
    ...
    function directoryUrl(type, district) {
        const page = TYPE_TO_PAGE[type];
        const qs = new URLSearchParams();
        if (district) qs.set('district_id', district);
        return page + (qs.toString() ? '?' + qs.toString() : '');
    }
    ...
    async function fetchDistrictRows(type, districtId) {
        const action = type === 'seller' ? 'sellers' : 'buyers';
        const response = await fetch(`api.php?action=${action}&district_id=${encodeURIComponent(districtId)}`);
    ...
    async function loadAll(type) {
        ...
        if (!window.app || typeof window.app.loadDistricts !== 'function') return [];
        const districts = await window.app.loadDistricts();
        const batches = await Promise.all(districts.map(d => fetchDistrictRows(type, d.id).catch(() => [])));
        cache[type] = batches.flat();
        return cache[type];
    }
    ...
    function ensureModal() {
        ...
        modal.id = 'directory-detail-modal';
        modal.className = 'modal';
        modal.setAttribute('role', 'dialog');
        modal.setAttribute('aria-modal', 'true');
        modal.innerHTML = `
            <div class="modal-backdrop" data-directory-close></div>
            <div class="modal-content directory-detail-modal-content">
                <div class="modal-header">
                    <h2 id="directory-detail-title">Contact</h2>
                    <button type="button" class="modal-close" aria-label="Close" data-directory-close>
                        <span class="material-symbols-rounded">close</span>
                    </button>
                </div>
                <div class="modal-body" id="directory-detail-body"></div>
            </div>`;
        document.body.appendChild(modal);
        modal.addEventListener('click', e => {
            if (e.target.closest('[data-directory-close]')) {
                if (window.app && typeof window.app.closeModal === 'function') window.app.closeModal(modal);
    ...
    function openDetail(row, type, shouldPush, district) {
        ...
        if (body) body.innerHTML = `
            <div class="directory-contact-hero">
                <div class="directory-avatar">${type === 'seller' ? '🌾' : '🏢'}</div>
                <div><span class="directory-role">${type === 'seller' ? 'Seller' : 'Buyer'}</span><h3>${esc(row.name)}</h3><p>${esc(row.district_name || '')}${row.address ? ' · ' + esc(row.address) : ''}</p></div>
            </div>
            ...
            <div class="directory-detail-actions">
                ${row.phone_number ? `<a class="directory-action primary" href="tel:${esc(row.phone_number)}"><span class="material-symbols-rounded">call</span> Call ${esc(row.phone_number)}</a>` : '<p class="directory-muted">No phone number listed.</p>'}
                ${row.email ? `<a class="directory-action" href="mailto:${esc(row.email)}"><span class="material-symbols-rounded">mail</span> Email</a>` : ''}
            </div>`;
        if (window.app && typeof window.app.openModal === 'function') window.app.openModal(m);
    ...
    function render(type, rows, district, shouldPush) {
        ...
        const districtOptions = [...districts.entries()].sort((a, b) => a[1].localeCompare(b[1]))
            .map(([id, name]) => `<option value="${esc(id)}" ${String(id) === String(district) ? 'selected' : ''}>${esc(name)}</option>`).join('');
        const title = type === 'seller' ? 'Find Sellers' : 'Find Buyers';
        const subtitle = type === 'seller' ? 'See sellers first, then narrow the list by district.' : 'See buyers first, then narrow the list by district.';

        area.innerHTML = `
            <section class="directory-page">
                <div class="directory-hero">
                    <div><span class="directory-eyebrow">AgroBusiness Malawi</span><h2>${title}</h2><p>${subtitle}</p></div>
                    <div class="directory-total"><strong>${rows.length}</strong><span>${type}s</span></div>
                </div>
                <div class="directory-controls">
                    <label class="directory-search"><span class="material-symbols-rounded">search</span><input id="directory-search" type="search" placeholder="Search ${type}s, crops, phone…" autocomplete="off"></label>
                    <label class="directory-district"><span>District</span><select id="directory-district"><option value="">All districts</option>${districtOptions}</select></label>
                </div>
                <div class="directory-count" id="directory-count"></div>
        ...
        function apply() {
            ...
            grid.innerHTML = filtered.length ? filtered.map(r => {
                const crop = String(r.crops_display || '').split(',').map(s => s.trim()).filter(Boolean);
                return `<button type="button" class="directory-card" data-directory-id="${Number(r.id)}">
                    <span class="directory-card-icon">${type === 'seller' ? '🌾' : '🏢'}</span>
                    <span class="directory-card-main"><strong>${esc(r.name)}</strong><small>${esc(r.district_name || '')}${r.address ? ' · ' + esc(r.address) : ''}</small><span class="directory-crops">${crop.slice(0, 4).map(c => `<em>${esc(c)}</em>`).join('')}</span></span>
                    <span class="material-symbols-rounded directory-card-arrow">chevron_right</span>
                </button>`;
        ...
        select.addEventListener('change', () => {
            pushDirectoryState(type, select.value);
            apply();
        });
        apply();
    }
    ...
    async function openDirectory(type, district, shouldPush) {
        try {
            ...
            render(type, rows, district || '', shouldPush);
        } catch (e) {
    ...
    async function replayPage() {
        ...
        const type = PAGE_TO_TYPE[page.replace('.php', '')];
        if (!type) return;
        const qs = new URLSearchParams(location.search);
        const id = Number(qs.get(type + '_id') || 0);
        const district = qs.get('district_id') || '';
        const rows = await loadAll(type);
        if (id) {
            const row = rows.find(r => Number(r.id) === id);
            if (row) openDetail(row, type, false, district);
            else render(type, rows, district, false);
        } else render(type, rows, district, false);
    }
    ...
    function install() {
        ...
        window.addEventListener('popstate', async e => {
            ...
            const type = PAGE_TO_TYPE[page.replace('.php', '')];
            if (!type) return;
            if (e.state && /_detail$/.test(e.state.view || '')) {
                const rows = await loadAll(type);
                const row = rows.find(r => Number(r.id) === Number(e.state.id));
                if (row) openDetail(row, type, false, e.state.district || '');
                else await openDirectory(type, e.state.district || '', false);
            } else await openDirectory(type, e.state && e.state.district ? e.state.district : '', false);
        });

        if (window.AGRO_PAGE === 'sellers' || window.AGRO_PAGE === 'buyers') {
            setTimeout(replayPage, 0);
        }
    }
