...
(function () {
    ...
    function ensureModal() {
        ...
        modal.addEventListener('click', e => { if (!e.target.closest('[data-directory-close]')) return; if (window.app && typeof window.app.closeModal === 'function') window.app.closeModal(modal); else modal.classList.remove('active'); if (history.state && /_detail$/.test(history.state.view || '')) history.back(); });
        return modal;
    }
    function openDetail(row, type, shouldPush, district) {
        const m = ensureModal(); if (shouldPush) pushDetailState(type, row, district);
        const title = document.getElementById('directory-detail-title'), body = document.getElementById('directory-detail-body');
